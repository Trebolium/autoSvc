...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ft", "--feat_type", type=str, help="name the model used for inferring"
    )
    parser.add_argument(
        "-um",
        "--use_model",
        type=str,
        default="worldHarmsOnly_pitchCond_Window608",
        help="name the model used for inferring",
    )
    parser.add_argument(
    ...
    if use_aper_feats == True:
        num_spectral_feats = (
            feat_params["num_harm_feats"] + feat_params["num_harm_feats"]
        )
    else:
    ...
    for sp in single_singer_list:
        ...
        if inputs.feat_type == "mel":
            db_unnormed_melspec = audio_to_mel_autovc(
                y, feat_params["fft_size"], hop_size, mel_filter
            )
            ...
            for subset in ["dev", "test", "train", None]:
                if subset == None:
                    raise FileNotFoundError(
                        f"Target file {fn} could not be found in pitch directory {pitch_dir}"
                    )
            ...
            try:
                if start_step < 0:
                    ...
                    midi_trimmed = midi_contour[
                        start_step : (start_step + window_timesteps)
                    ]
