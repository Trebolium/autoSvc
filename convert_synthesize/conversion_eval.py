...
def matching_pitch_clip(
    ...
    track_search_tolerance=10,
    voiced_percent_tolerance=0.7,
):
    matched_singer_found = False
    attempt_num = 0
    while matched_singer_found == False:
        trg_path, trg_rand_gend_int = get_song_path(
            trg_gender,
            gender_separated_lists,
            os.path.join(this_train_params.SVC_feat_dir, subset),
        )
        ...
        trg_spec_feats, trg_pitch_feats = get_feats(
            trg_path, this_train_params.pitch_dir, subset, this_train_params.midi_range
        )
        continuous_pitch_feats = np.argmax(trg_pitch_feats, axis=1)
        average_trg_pitches = convert_utils.get_relevant_avg_pitches(
            continuous_pitch_feats, this_train_params.window_timesteps
        )
        start_of_chunk_idx = convert_utils.best_pitch_matching_idx(
            average_trg_pitches, avg_src_pitch
        )

        if start_of_chunk_idx >= 0:
            trg_pitch_clip, _ = fix_feat_length(
                trg_pitch_feats,
        ...
        if attempt_num >= track_search_tolerance:
            raise convert_utils.NoMatchError(
...
def pitch_matched_src_trg(
    ...
    while not matching_target_found:
        src_path, src_rand_gend_int = get_song_path(
            src_gender,
            gender_separated_lists,
            os.path.join(this_train_params.SVC_feat_dir, subset),
        )
        src_spec_feats, src_pitch_feats = get_feats(
            src_path, this_train_params.pitch_dir, subset, this_train_params.midi_range
        )
        ...
        src_pitch_clip, _ = fix_feat_length(
            src_pitch_feats, this_train_params.window_timesteps, offset=src_rand_ts
        )
        ...
        print(
            f"src_song: {os.path.basename(src_path)}, rand_int: {src_rand_ts}, src_gend: {gend_dict[src_gender]}, avg_src_pitch: {avg_src_pitch}"
        )

        print(avg_src_pitch)
        try:
            spec_pitch_gendint_randts_path = matching_pitch_clip(
                trg_gender,
                avg_src_pitch,
                src_path,
                this_train_params,
            ...
            (
                ...
                trg_rand_ts,
                trg_path,
            ) = spec_pitch_gendint_randts_path
            try:
...
def get_svc_tensors(src_gender, trg_gender, subset_metadata, gender_separated_lists):
    ...
    (
        ...
        src_randts,
        src_fn,
    ) = parse_data(src_data, subset_metadata, subset_names, device)
    (
        ...
        trg_randts,
        trg_fn,
    ) = parse_data(trg_data, subset_metadata, subset_names, device)
...
def make_gen_dependancies(model_name):
    ...
    gender_separated_lists = convert_utils.get_gender_lists(
        os.path.join(this_train_params.SVC_feat_dir, subset)
    )
...
for i in range(num_listening_studies):
    for j, model_name in enumerate(model_names.keys()):
        ...
        G, SIE, subset_metadata, gender_separated_lists = make_gen_dependancies(
            model_name
        )
        ...
        for k, (src_gender, trg_gender) in enumerate(gender_conds):
            ...
            (
                ...
                src_gender, trg_gender, subset_metadata, gender_separated_lists
            )
            ...
            cosine_sim = np.dot(converted_sie_emb, trg_emb_arr) / (
                norm(converted_sie_emb) * norm(trg_emb_arr)
            )
            print(cosine_sim)
            if cosine_sim > 1:
                raise Exception
            euclidean_distance = np.linalg.norm(converted_sie_emb - trg_emb_arr)
            results_condition.append(
                (
                    cosine_sim,
                    euclidean_distance,
                    (j, model_name),
                    (k, (src_gender, trg_gender)),
                )
            )
