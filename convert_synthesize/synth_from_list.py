...
parser.add_argument(
    ...
    "--audio_dir",
    type=str,
    default="/homes/bdoc3/my_data/audio_data/damp_desilenced_concat",
    help="name the model used for inferring",
)
parser.add_argument(
    "-ae",
...
parser.add_argument(
    "-ss", "--subset", type=str, default="", help="name the model used for inferring"
)
parser.add_argument(
    "-wc", "--which_cuda", type=int, default=0, help="Determine which cuda to use"
)
parser.add_argument(
    "-ns", "--num_singers", type=int, default=4, help="Determine which cuda to use"
)
parser.add_argument(
...
if "mel" in config.use_model or "Mel" in config.use_model:
    print(
        "Are you using the correectly pretrained WaveNet, and have you set the hparams file accordingly?"
    )
...
if use_avg_singer_embs:
    ...
    subset_singer_metadata = pickle.load(
        open(
            os.path.join(
                metadata_path,
                os.path.basename(SIE_path),
                os.path.basename(SIE_feat_dir)
                + f"_{config.subset}_singers_metadata.pkl",
            ),
            "rb",
        )
    )
...
for row in reader:
    row[0] = os.path.join(
        SVC_feat_dir, "/".join(row[0].split("/")[-3:])
    )  # fix this messiness later
...
audio_fps = [
    fp for fp in audio_fps if fp.endswith(config.audio_ext) and not fp.startswith(".")
]
audio_list = []
...
for i, entry in enumerate(fp_gender_list):
    ...
    if not fpgl_missing_timestep:
        SVC_feats, start_step = fix_feat_length(
            SVC_feats, window_timesteps, int(entry[2])
        )
    else:
    ...
    singer_id = (
        gender
        + "-"
        + os.path.basename(fp)[:-4]
        + "_"
        + str(int(start_step * SVC_feat_params["frame_dur_ms"] / 1000))
        + "s"
    )
...
for i, (
    singer_id_src,
    midi_contour_npy_src,
    SVC_input_feats_npy_src,
    SIE_input_feats_npy_src,
    aper_npy_src,
) in enumerate(feat_list):
    for j, (
        singer_id_trg,
        midi_contour_npy_trg,
        SVC_input_feats_npy_trg,
        SIE_input_feats_npy_trg,
        aper_npy_trg,
    ) in enumerate(feat_list):
        ...
        if use_avg_singer_embs:
            ...
            timbre_src_tns = (
                torch.from_numpy(timbre_src_npy).to(device).float().unsqueeze(0)
            )
            ...
            timbre_trg_tns = (
                torch.from_numpy(timbre_trg_npy).to(device).float().unsqueeze(0)
            )
        ...
        # src feats to SVC
        SVC_input_feats_tns_src = (
            torch.from_numpy(SVC_input_feats_npy_src).to(device).float().unsqueeze(0)
        )

        # src pitch to SVC
        if SVC_pitch_cond:
            onehot_midi_npy_src = utils.get_onehot_midi(
                midi_contour_npy_src, midi_range
            )
            onehot_midi_tns_src = (
                torch.from_numpy(onehot_midi_npy_src).to(device).float().unsqueeze(0)
            )
            feat_tns_prnt, feat_tns_psnt, code_real, _, _ = G(
                SVC_input_feats_tns_src,
                timbre_src_tns,
                timbre_trg_tns,
                onehot_midi_tns_src,
            ...
            feat_tns_prnt, feat_tns_psnt, code_real, _, _ = G(
                SVC_input_feats_tns_src, timbre_src_tns, timbre_trg_tns
            )

        # recently changed loss function from old to new. Make sure you are using the right one
        if config.old_loss:
            feat_recon_npy = (
                (feat_tns_prnt + feat_tns_psnt).squeeze(1)[0].cpu().detach().numpy()
            )
        else:
        ...
        converted_feats.append(
            (converted_id, feat_recon_npy, aper_npy_src, midi_contour_npy_src)
        )
...
for i, (converted_id, x_identic, aper_src, onehot_midi_src) in tqdm(
    enumerate(converted_feats)
):
    if (
...
for singer_id, y_chunk in audio_chunk_list:
    sf.write(
        os.path.join(wav_dir, singer_id + ".wav"),
        y_chunk,
        samplerate=SVC_feat_params["sr"],
    )
