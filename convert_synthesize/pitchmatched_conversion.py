...
from .. import utils
from convert_utils import get_gender_lists, best_pitch_matching_idx, get_relevant_avg_pitches, NoMatchError
...
def get_feats(path):
    ...
    if 'crepe' in pitch_dir:
        crepe_data = np.load(os.path.join(pitch_dir, subset, fn.split('_')[0], fn[:-4]+'.npz'))
...
def matching_pitch_clip(trg_gender, avg_src_midi, src_path, pitch_match, track_search_tolerance=11, voiced_percent_tolerance=0.7):
    ...
    while matched_singer_found==False:
        ...
        trg_midi_onehot = midi_as_onehot(trg_midi_continuous, this_train_params.midi_range)

        if pitch_match:
            average_trg_pitches = get_relevant_avg_pitches(trg_midi_continuous, this_train_params.window_timesteps)
            start_of_chunk_idx = best_pitch_matching_idx(average_trg_pitches, avg_src_midi)
        else:
            start_of_chunk_idx = random.randint(0,len(trg_midi_continuous[:-this_train_params.window_timesteps]))
        
        # get pitch of trg and compute distance from src
        if start_of_chunk_idx >= 0:
            trg_midi_continuous_clip, _ = fix_feat_length(trg_midi_continuous, this_train_params.window_timesteps, offset=start_of_chunk_idx)
            trg_midi_onehot_clip, _ = fix_feat_length(trg_midi_onehot, this_train_params.window_timesteps, offset=start_of_chunk_idx)
            ...
            print('avg_trg_pitch', np.average(trg_midi_continuous_clip), 'avg_src_midi', avg_src_midi, 'octave_diff', number_of_octave_diff,)
    ...
    return trg_spec_clip, trg_midi_onehot_clip, trg_rand_gend_int, start_of_chunk_idx, trg_path, number_of_octave_diff
    
    
def pitch_matched_src_trg(src_gender, trg_gender, voiced_percent_tolerance, pitch_match=True):
    ...
    #     src_path = '/import/c4dm-02/bdoc3/spmel/damp_qianParams/test/434587164/434587164_2141814685.npy'
        ...
        src_midi_onehot = midi_as_onehot(src_midi_continous, this_train_params.midi_range)
    ...
    #     src_rand_ts = 2981
        ...
        src_midi_continuous_clip, _ = fix_feat_length(src_midi_continous, this_train_params.window_timesteps, offset=src_rand_ts)
        ...
        print(f'src_song: {os.path.basename(src_path)}, rand_int: {src_rand_ts}, src_gend: {gend_dict[src_gender]}, avg_src_midi: {avg_src_midi}')

        try:
            spec_pitch_gendint_randts_path_pitchdiff = matching_pitch_clip(trg_gender,
                                                    avg_src_midi,
                                                    src_path,
                                                    pitch_match,
                                                    voiced_percent_tolerance=voiced_percent_tolerance)

            trg_spec_clip, trg_midi_onehot_clip, trg_rand_gend_int, trg_rand_ts, trg_path, octave_pitch_diff = spec_pitch_gendint_randts_path_pitchdiff
            try:
    ...
    if not pitch_match:
        ...
        adjusted_src_midi_continuous_clip[voiced]  = adjusted_src_midi_continuous_clip[voiced] + (12 * octave_pitch_diff) # plus becomes minus if octave_pitch_diff is neg
        print('adjusted_avg_src_midi', round(np.average(adjusted_src_midi_continuous_clip[voiced])))
    ...
    src_midi_onehot_clip = midi_as_onehot(adjusted_src_midi_continuous_clip, this_train_params.midi_range)
...
def collect_feats_convert_synth(src_gender, trg_gender, self_convert, pitch_match):
    ...
    src_data, trg_data = pitch_matched_src_trg(src_gender, trg_gender, voiced_percent_tolerance, pitch_match=pitch_match)
...
parser.add_argument('-sc', '--self_convert', type=int, default=0) # caution: self-convert means get autovc to recreate its input
parser.add_argument('-ro', '--resynth_orgs', type=int, default=0) # synthesize the source and target audio from unprocessed mels
...
if len(config.cond_list) != 0:
    ...
    for j, model_name in enumerate(model_names.keys()):
        ...
        for i, (src_gender, trg_gender) in enumerate(gender_conds):
            ...
            collect_feats_convert_synth(src_gender, trg_gender, config.self_convert, config.pitch_match)