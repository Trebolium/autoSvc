...
parser.add_argument(
    ...
    "--audio_dir",
    type=str,
    default="../singer-identity-encoder/example_feats/train",
...
audio_fps = [
    fp
    for fp in os.path.abspath(audio_fps)
...
for i, entry in enumerate(fp_gender_list):
    ...
    singer_id = (
        ...
        + fn
        + "_"
        + str(int(int(entry[2]) * feat_params["frame_dur_ms"] / 1000))
        + "s"
    )
    ...
    if config.feat_type == "mel":
        final_feats = audio_to_mel_autovc(
            y_chunk, feat_params["fft_size"], hop_size, mel_filter
        )
...
for entry in audio_chunk_list:
    ...
    sf.write(
        os.path.join(wav_dir, entry[0] + "_audio2feats2audio.wav"),
        waveform,
        samplerate=feat_params["sr"],
    )
