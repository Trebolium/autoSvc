...
def fp_gendered_list_from_vctk_txt():
    ...
    with open('/import/c4dm-datasets/VCTK-Corpus-0.92/speaker-info.txt', 'r') as in_file:
    ...
    try:
        chosen_spkrs = random.sample(males, k=each_gender) + random.sample(females, k=each_gender)
    except ValueError as e:
        Exception(f'No entries in gender lists led to error {e}. Consider checking that the contents of the subset_names match those of the VCTK text file')
...
def calculate_transposed_pitch(pitch_for_trans, src_avg_pitch, trg_avg_pitch):
    ...
    if avg_pitch_diff >0: # then src is higher than trg and might have to go down
        ...
        src_avg_octaves = [src_avg_pitch] + [src_avg_pitch * (1/2*i) for i in range(1, 8)]

    else: # then src is lower than trg and might have to go up

        src_avg_octaves = [src_avg_pitch] + [src_avg_pitch * (2*i) for i in range(1, 8)]
...
def setup_variables():
    ...
    if 'mel' in SVC_feat_dir.lower():
        SVC_is_mel = True
        SVC_mel_filter = mel(SVC_feat_params['sr'], SVC_feat_params['fft_size'], fmin=SVC_feat_params['fmin'], fmax=SVC_feat_params['fmax'], n_mels=SVC_feat_params['num_harm_feats']).T
        SVC_min_level = np.exp(-100 / 20 * np.log(10))
        SVC_hop_size = int((SVC_feat_params['frame_dur_ms']/1000) * SVC_feat_params['sr'])
    ...
    this_model_dir = os.path.join(saved_models_dir, os.path.basename(vc_dir), svc_model_name)
...
if __name__ == '__main__':
    ...
    setup_variables()
    metadata_path = os.path.join(metadata_root_dir, sie_model_name, f'{SIE_dataset_name}_{subset}_singers_metadata.pkl')
    ...
    if os.path.basename(vc_dir) == 'basic_autovc':
        checkpoint_path = os.path.join(this_model_dir, 'ckpts', 'ckpt_' +str(ckpt_iters) +'.pth.tar')
        config = pickle.load(open(os.path.join(this_model_dir, svc_model_name, 'config.pkl'), 'rb'))
    ...
    if (SVC_feat_dir != SIE_feat_dir) and not use_avg_singer_embs:
        raise Exception('Code is not preppared to handle 2 difference inputs while not using avg embs')
    
    SIE_feat_params, SVC_feat_params, determine_dim_size(SIE_feat_params, SVC_feat_params, SIE_feat_dir, SVC_feat_dir, use_aper_feats)
    
    if not use_avg_singer_embs:
        sie, sie_num_feats_used = utils.setup_sie(device, loss_device, os.path.join(sie_dir, sie_model_name, adam_init))


    # Load Generator model
    if os.path.basename(vc_dir) == 'basic_autovc':
        G = model_vc.Generator(dim_neck, dim_emb, dim_pre, sample_freq).eval().to(device)
    # if autosvc used, a lot more options available
        ...
        G, _, _ = utils.setup_gen(dim_neck, dim_emb, dim_pre, sample_freq, SVC_num_feats, pitch_dim, device, checkpoint_path, adam_init)
    ...
    for entry in chosen_spkrs:
        ...
        for fp in tqdm(fps):
            ...
            voiced = pitch_info[:,1].astype(int) == 0 # remove the interpretted values generated because of unvoiced sections
        ...
        SVC_feats = np.load(os.path.join(SVC_feat_dir, subset, voice_dir, uttr_name +npy_ext))
        SIE_feats = np.load(os.path.join(SIE_feat_dir, subset, voice_dir, uttr_name +npy_ext))
        pitch_feats = np.load(os.path.join(pitch_dir, subset, voice_dir, uttr_name +npy_ext))[:,-2:]
        chosen_spkr_name_feats.append((uttr_name, SVC_feats, SIE_feats, pitch_feats, singer_pitch_mean))
    ...
    for i in range(len(chosen_spkr_name_feats)):
        ...
        if use_avg_singer_embs:
            ...
            src_emb = subset_metadata[subset_names.index(src_voice_dir)][1] #subset_n and subset_m are paired indices.
            src_emb = torch.from_numpy(src_emb).to(device).float().unsqueeze(0)

        else:

            src_sie_feats = torch.from_numpy(src_sie_feats).to(device).float().unsqueeze(0)
        ...
        for j in range(len(chosen_spkr_name_feats)):
            ...
            if use_avg_singer_embs:
                ...
                trg_emb = subset_metadata[subset_names.index(trg_voice_dir)][1] #subset_n and subset_m are paired indices.
                trg_emb = torch.from_numpy(trg_emb).to(device).float().unsqueeze(0)

            else:

                trg_sie_feats = torch.from_numpy(trg_sie_feats).to(device).float().unsqueeze(0)
            ...
            if SVC_pitch_cond:
                ...
                _, feat_tns_psnt, _, _, _ = G(src_specs, src_emb, trg_emb, onehot_midi_tns_src)
            ...
            voiced = src_pitch[:,1].astype(int) == 0 # remove the interpretted values generated because of unvoiced sections
            ...
            trg_pitch = calculate_transposed_pitch(src_pitch, src_avg_pitch, trg_avg_pitch)
    ...
    if include_unprocessed_audio:
        for i in range(len(org_audio_paths)):
            ...
            if SVC_is_mel:
                feats = raw_audio_to_mel_autovc(y, SVC_mel_filter, SVC_min_level, SVC_hop_size, SVC_feat_params)
