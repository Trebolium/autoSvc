...
class DuoFeatureDataset(Dataset):
    def __init__(
        ...
        if chosen_class_num != None:
            ...
            if ds_size == 1.0:
                feats1_subset_dir = os.path.abspath(
                    os.path.join(self.SIE_feat_dir, subset_name)
                )
                _, feats1_fps = recursive_file_retrieval(
                    feats1_subset_dir, ignore_hidden_dirs=True
                )
            else:
                feats1_subset_dir = os.path.abspath(
        ...
        numpy_fns = [
            os.path.basename(fp)
            for fp in sorted(feats1_fps)
            if fp.endswith(self.ext) and not fp.startswith(".")
        ]
    ...
    def __getitem__(self, index):
        ...
        spec1, spec2 = (
            feats1_features[:, : self.feats1_num_feats],
...
def generate_loaders(datasets, ds_labels):
    ...
    for i, ds in enumerate(datasets):
        random.seed(
            1
        )  # reinstigating this at every iteration ensures the same random numbers are for each dataset
    ...
    with open("dataset_ids_train_idxs.pkl", "wb") as File:
        pickle.dump(
            ds_ids_train_idxs, File
        )  # save dataset ids as pkl for potential hindsight analysis
    return val_loaders
