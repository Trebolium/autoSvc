...
class AutoSvc(object):
    def __init__(self, SIE_params, SVC_params):
        ...
        if (
            SVC_feat_dir == ""
        ):  # if svc dir isn't established, then its ok to pass it same feats as SIE
        ...
        self.train_singer_metadata = pickle.load(
            open(
                os.path.join(
                    metadata_path,
                    os.path.basename(SIE_model_path),
        ...
        self.val_singer_metadata = pickle.load(
            open(
                os.path.join(
                    metadata_path,
                    os.path.basename(SIE_model_path),
        ...
        if subset_size == 1.0:
            ...
            self.train_iter_multiple = self.get_iter_size(
                SVC_feat_dir, "train" + f"/.{subset_size}_size"
            )
            self.val_iter_multiple = self.get_iter_size(
                SVC_feat_dir, "val" + f"/.{subset_size}_size"
            )
    ...
    def plot_comparisons(self, feat_npys, i):
        ...
        ex_path = os.path.join(
            SVC_models_dir, SVC_model_name, "image_comparison", f"step_{i}"
        )
    ...
    def batch_iterate(
        ...
        for _ in range(iter_multiple):
            ...
            for i in range(
                this_cycle_initial_step + 1,
                (this_cycle_initial_step + 1 + num_loader_iters),
            ):  # using +1 because of some scheduler sync issues (quick hack)
                interlog_counter += 1
                ...
                if self.make_examples:
                    ...
                    ex_path = os.path.join(
                        ...
                        SVC_model_name,
                        "input_tensor_plots",
                        f"STEP-{i}_ID-{example_id[0][:-4]}_TIMESTAMP-{start_step[0]}",
                    )
                ...
                onehot_midi_input = (
                    torch.from_numpy(onehot_midi_npy).to(self.device).float()
                )
                    ...
                    for j, ex_id in enumerate(example_id):
                        ...
                        try:
                            if j == 0:
                                emb_org = (
                                    torch.from_numpy(
                                        singer_metadata[singermeta_index][1]
                                    )
                                    .to(self.device)
                                    .float()
                                    .unsqueeze(0)
                ...
                x_identic_prnt, x_identic_psnt, code_real, _, _ = self.G(
                    SVC_input, emb_org, emb_org, onehot_midi_input
                )
                ...
                recon_loss = (prnt_loss_weight * g_loss_id_prnt) + (
                    psnt_loss_weight * g_loss_id_psnt
                )
                ...
                if use_emb_loss:
                    ...
                    cc_emb_loss = (
                        cc_emb_loss.detach().clone()
                    )  # we don't want this loss to backprop through self.sie
                else:
                    cc_emb_loss = 0
                ...
                if mode == "train":
                    ...
                    if i % spec_freq == 0:
                        feat_npys = [
                            np.rot90(onehot_midi_npy[0]),
                            np.rot90(SIE_feats_npy[0]),
                ...
                if i % log_step == 0:
                    ...
                    if i % ckpt_freq == 0:
                        dst_path = os.path.join(
                            SVC_models_dir, SVC_model_name, f"ckpt_{i}.pt"
                        )
                        save_dict = {
                            "step": self.train_latest_step,
                            "model_state": self.G.state_dict(),
        ...
        if mode.startswith(
            ...
            if check == "lowest_loss":
                ...
                print(
                    f"Saved model (step {self.train_latest_step}) at time {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}"
                )
            elif check == 0:
    ...
    def logs(self, intercycle_losses_dict, mode):
        for j in range(len(intercycle_losses_dict)):
            ...
            self.writer.add_scalar(
                f"{loss_name}_loss_id_psnt_{use_loader}/{mode}",
                this_loss,
                self.train_latest_step,
            )
        self.writer.flush()
    ...
    def train(self, train_loader, val_loaders):
        while self.train_latest_step < max_iters:
            ...
            print("TRAIN REPORT\n")
            self.G.train()
            intercycle_losses_dict = self.batch_iterate(
                mode,
                train_loader,
                self.train_latest_step,
                self.train_singer_metadata,
                num_loader_iters,
            )
            ...
            for ds_label, val_loader in val_loaders:
                ...
                self.G.eval()
                with torch.no_grad():
                    cycle_loss = self.batch_iterate(
                        mode, val_loader, 0, self.val_singer_metadata, num_loader_iters
                    )
