...
svc_model_name = (
    "damp_mel_Size0.25-avgEmbs_CcLoss__-bestPerformingSIE_mel80-to500kIters-Cont2"
)
...
this_svc_model_dir = os.path.join(
    saved_models_dir, os.path.basename(vc_verion), svc_model_name
)
...
SIE_feats_params, SVC_feats_params = utils.determine_dim_size(
    SIE_feats_params, SVC_feats_params, SIE_feat_dir, SVC_feat_dir, use_aper_feats=False
)
train_dataset, train_loader = load_primary_dataloader(
    SIE_feats_params,
    subset,
    SVC_feats_params,
    ds_size,
...
if testing_with == "bneck":
    ...
    h_layer_outs = [sie_emb_size // 2]

vc_classer = (
    Aux_Voice_Classifier(sie_emb_size, h_layer_outs, len(train_dataset))
    .to(device)
    .train()
)
        ...
        for i, mb in enumerate(train_loader):
            ...
            if use_avg_embs:
                for l, id in enumerate(example_id):
                    ...
                    if l == 0:
                        emb_org = (
                            torch.from_numpy(subset_metadata[singermeta_index][1])
                            .to(device)
                            .float()
                            .unsqueeze(0)
                        )
            ...
            if testing_with == "bneck":
                x_identic_prnt, x_identic_psnt, code_real, _, _ = G(
                    SVC_feats.to(device).float(), emb_org, emb_org, None
                )
            ...
            if this_step % log_step == 0:
                ...
                log = "Elapsed [{}], Mode {}, Iter [{}/{}]".format(
                    ...
                    "train",
                    this_step,
                    (num_epochs) * (ex_per_spkr) * len(train_loader),
                )
